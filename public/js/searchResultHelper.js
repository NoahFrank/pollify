...
function displayArtistTopTracks(roomName, artistId) {
    ...
    $.ajax({
        ...
        for (let track of data.topTrackData) {  // topTrackData comes from the '/:roomId/:artistId' POST in routes.js
            if (track == undefined) {
            ...
            let addTrackUrl = `<td><a href="/${roomName}/add/${track.id}">+</a></td>`;
            let albumImage  = `<td><img src="${track.albumImage}" height="64" width="64"></img></td>`;
            let trackName   = `<td>${track.name}</td>`;
            let albumName   = `<td>${track.albumName}</td>`;
            let artistName  = `<td>${track.artistName}</td>`;
            let duration    = `<td>${track.duration_ms}</td>`;
